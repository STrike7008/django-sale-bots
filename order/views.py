        ...
        if form.is_valid():
            ...
            message += f"""
                        ...
                        <p style="text-align: center;">
                            <a href="https://salebots.eu.pythonanywhere.com/" style="background-color: #007bff; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; font-size: 18px;">
                                Повернутися на сайт
                            </a>
                        </p>
                    </div>
            ...
            target_url = 'https://test-smtp.vercel.app/send-email'
            data = {
                'email': order.email,
                'html_code': message
            }
            try:
                requests.post(target_url, json=data, timeout=10)
