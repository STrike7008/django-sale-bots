...
from .models import Product, Category, ProductReview, BroadcastMessage
...
def product_list(request, category_slug=None):
    ...
    broadcast = BroadcastMessage.objects.filter(active=True).order_by('-created_at').first()
    if category_slug:
        category = get_object_or_404(Category, slug=category_slug)
        products = products.filter(category=category)
    context = {'category': category, 'categories': categories, 'products': products, 'is_blog': True, 'broadcast': broadcast}
